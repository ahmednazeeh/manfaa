 ...
 * path): a staff member keys in a sale by hand. The server owns every rule
 * — rate frozen at occurred_at, ceiling rounding, below-minimum zeroing,
 * stale-timestamp hold, duplicate-invoice rejection. This screen confirms
 * the customer by masked name first (§11 phone-recycling control) and
 * previews the cost as an ESTIMATE at the current rate only.
 *
 ...
 *
 * This is the ONLY implementation of that experience. The /credit route
 * renders it, and so may a dialog on another screen; the component never
 * learns which, so the two hosts cannot drift into telling merchants
 * different things. A host says only how it is embedded (`variant`) and
 * what happens afterwards (`onDone` / `onCancel`); the form tells the host
 * back the one thing only it can see, whether anything has been typed into
 * it yet (`onDirtyChange`), so a host that can close over the top of a
 * half-keyed sale can ask first.
 */
...
/**
 * What a recorded credit LOOKS like, once — the confirmation the form shows
 * after a sale is keyed in, and the one a host that closes on success (a
 * dialog) shows in its place. Exported for that second host only: a screen
 * that hands the credit form a way out must still say exactly what the
 * /credit route says about what was recorded, down to the wording of a
 * below-minimum or on-hold sale.
 */
export function CreditResultCard({
  result,
  categories,
  onReset,
  onDismiss,
  ...
  onReset: () => void;
  /**
   * Put this confirmation away without keying another sale. Offered only by
   * a host where the two are different acts: on the route "Credit another
   * customer" already leaves the empty form on screen, but on a dashboard
   * it re-opens the dialog, so a cashier who has read the confirmation and
   * wants their figures back needs a way that is not a credit.
   */
  onDismiss?: () => void;
  /**
   * The card's own spacing, because where it sits differs: above the form
   * on the route (the default `mb-5`), one item in a spaced column on a
   * dashboard (pass `''` and let the column's gap do it).
   */
  className?: string;
}) {
...
/**
 * How much ROOM the host gives the form — deliberately not what the host
 * IS, so the component never branches on sitting in a page or a dialog.
 * `wide` puts the entry card and the cost preview side by side on a large
 * screen; `narrow` stacks them in one column. The fields, the rules, the
 * request and the result are identical under both.
 */
export type CreditCustomerFormVariant = 'wide' | 'narrow';

export interface CreditCustomerFormProps {
  /** Layout hint only — see CreditCustomerFormVariant. Defaults to `wide`. */
  variant?: CreditCustomerFormVariant;
  /**
   * Called once the server has recorded a credit, with the transaction and
   * the customer it was confirmed against. The form has already reset
   * itself and is showing its result card by the time this runs, so a host
   * that keeps it mounted needs to do nothing; a host that is a dialog can
   * close on it and say its own piece.
   */
  onDone?: (result: CreditResult) => void;
  /**
   * Renders a Cancel control beside the submit button. Omitted where there
   * is nothing to cancel out of, as on the route itself.
   */
  onCancel?: () => void;
  /**
   * Whether there is anything typed in here worth protecting — fired on
   * every change to that answer. Only the form can see its own fields, so
   * only the form can answer it, and answering it HERE means every host
   * that can throw the entry away (a dialog's Cancel, its X) agrees on what
   * "half-typed" means.
   *
   * Pass a stable callback: it is an effect dependency.
   */
  onDirtyChange?: (dirty: boolean) => void;
  /**
   * Whether a credit is currently WITH THE SERVER. A host that can unmount
   * this form (a dialog) must refuse to do so while this is true: the
   * success handling hangs off the mutate() call, so unmounting mid-request
   * loses the answer to a sale that is being recorded anyway. The form
   * shuts its own Cancel for the same window; the host owns the rest.
   *
   * Pass a stable callback: it is an effect dependency.
   */
...
export function CreditCustomerForm({
  variant = 'wide',
  onDone,
  onCancel,
  onDirtyChange,
  ...
  // The sale time is PREFILLED, so "does it differ from now" is not a
  // question about typing — it drifts by itself as the clock moves. Whether
  // the cashier touched it is, and that is what a discard warning is about.
   ...
   *
   * "Edited" is derived from the VALUE, never from the act: putting the
   * field back to the prefilled now — which is exactly what the "set to
   * now" link does — leaves nothing a discard could cost anyone, so it must
   * not arm the discard question.
   */
  const changeOccurredAt = (value: string) => {
    setOccurredAt(value);
    setOccurredAtEdited(value !== nowLocalValue());
    setBackdatedConfirmed(false);
  };

  /**
   * Is there entry in here that throwing the form away would cost someone?
   * Every field a cashier can put something into, plus the two controls
   * that are themselves a decision (the split toggle, the backdated tick) —
   * the split rows count only once an amount is typed, because an empty row
   * is what a fresh form already looks like.
   */
  const dirty =
    code !== '' ||
    invoiceNo !== '' ||
    ...
    overrideOpen ||
    splitEnabled ||
    splitRows.some((row) => row.amount.trim() !== '') ||
    backdatedConfirmed ||
    occurredAtEdited;
  ...
  /**
   * A credit is IN FLIGHT. The host is told because only the host owns the
   * ways out, and a way out taken now would unmount this form mid-request:
   * the success callback below lives on the mutate() call, so unmounting
   * kills it while the server still records the sale. The merchant would be
   * told nothing — and the discard question says the opposite. So the exits
   * are shut for exactly as long as this is true.
   */
  const busy = createCredit.isPending;
  useEffect(() => {
    onBusyChange?.(busy);
  }, [busy, onBusyChange]);
  ...
  const submit = () => {
    ...
    createCredit.mutate(
      ...
      {
        onSuccess: (response) => {
          const recorded: CreditResult = {
            transaction: response.data,
            customerName,
          };
          setResult(recorded);
          resetForm();
          // The host's turn: a page keeps showing the result card, a dialog
          // closes. Last, so the form is already reset and settled either way.
          onDone?.(recorded);
        },
      },
    );
  ...
  return (
    <>
      ...
      <div
        className={
          variant === 'wide'
            ? 'grid grid-cols-1 lg:grid-cols-3 gap-5 pb-7.5 items-start'
            : 'grid grid-cols-1 gap-5 items-start'
        }
      >
        <Card className={variant === 'wide' ? 'lg:col-span-2' : undefined}>
          <CardHeader>
          ...
          <CardContent className="flex flex-col gap-5">
            ...
            <div className="flex flex-wrap items-center justify-end gap-3">
              ...
              {onCancel && (
                // Shut while the sale is with the server, for the same
                // reason the submit button is: there is no such thing as
                // half a credit, and walking out now would leave the
                // merchant with no statement of what was recorded.
                <Button variant="outline" disabled={busy} onClick={onCancel}>
                  {t('common.cancel')}
                </Button>
              )}
              {/* Named from the same key the discard copy quotes, so a
                  Dhivehi merchant is pointed at the words on the button. */}
