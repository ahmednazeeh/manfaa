...
/// sign-in is even considered. Offline is not an error here — a till with no
/// signal still opens on its cached session, so a failed config fetch routes
/// by session and the gate is enforced on the next connected launch.
...
class _BootScreenState extends ConsumerState<BootScreen> {
  ...
  Future<void> _boot() async {
    ...
    if (session.signedIn) {
      // Fresh permissions AND fresh status on every launch (guide §3) —
      // fire-and-forget: the answer lands in the session, the revision
      // bumps, and the router re-routes if the status or role changed.
      // Offline must never block the till, so failures are swallowed.
      unawaited(
        ref.read(apiProvider).me().then((_) {}, onError: (Object _) {}),
      );
    }
    ...
    try {
      ...
      next = session.signedIn ? homeLocationFor(session) : '/login';
    } on Exception {
      next = session.signedIn ? homeLocationFor(session) : '/login';
    }
        ...
        child: Column(
          ...
          children: [
            ...
            if (storeUrl.isNotEmpty) ...[
              ...
              // download link is at least visible and copyable. Links wear
              // the violet accent.
