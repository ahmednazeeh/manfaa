 ...
 * items are frozen in that window, so the file re-renders byte-identical
 * and a download that died mid-transfer never strands the batch. The moment
 * a result import touches any item, export closes: from there a fresh file
 * would diverge from the one the bank is acting on.
 */
final readonly class BankFileExporter
{
    public function __construct(private BankFileFormatter $formatter = new GenericCsvFormatter) {}
